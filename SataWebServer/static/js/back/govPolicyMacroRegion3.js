...
function initChart4(){
	...
	// 使用刚指定的配置项和数据显示图表。
	myChart.setOption(option);  // set chart 6
...
function updateChart1(selectRegion, selectTime, selectIndust){
...
function updateChart3(selectRegion, selectTime, selectIndust){
	$.post("govPolicyMacroRegion/comNum", {'region':selectRegion, 'time':selectTime, 'industType':selectIndust }, function(ret){
			legendData = ret['legendData']
			yAxisData = ret['yAxisData']
			seriesData = ret['seriesData']
...
function updateBarChartY(xAxisData, legendData, seriesData, chartId){
	sData = []
	//alert(seriesData)
...
} // end of updateBarChartY

function updateRadarChart(xAxisData, legendData, seriesData, chartId){
	// 基于准备好的dom，初始化echarts实例
	var myChart = echarts.init(document.getElementById(chartId));
	sData = []
	for (i=0; i<seriesData.length ;i++ )
...
} // end of updateRadarChart

function updateBarChart(xAxisData, legendData, seriesData, chartId){
...
function updateBarChart1(xAxisData, legendData, seriesData, chartId){
	var myChart = echarts.init(document.getElementById(chartId));
	sData = []
	//alert(increaseSData);
	if (xAxisData.length == 1)
...
} // end of updateBarChart1

function updatePieChart(xAxisData, legendData, seriesData, seriesData1, chartId, name){
	...
	for (i=0; i<seriesData.length ;i++ )
	{
		d = {value:seriesData[i], name:xAxisData[i]}
		sData.push(d)
	}
	...
	for (i=0; i<seriesData.length;i++ )
	{
		for (j=0;j<seriesData1.length ;j++ )
		{
			d = {value:seriesData1[j][i], name:legendData[j]}
			sData1.push(d)
		}
	}
...
// for chart9
function updateRadarChart1(xAxisData, legendData, regionSumMin, regionSumMax, regionSum, chartId){  
	// 基于准备好的dom，初始化echarts实例
	alert(xAxisData);
	alert(regionSum);
	var myChart = echarts.init(document.getElementById(chartId));
	...
	//    },
		...
		tooltip: {
			...
			formatter: function (params) {
				var id = params.dataIndex;
				return xAxisData[id] + '<br>最低：' + data[id][0] + '<br>最高：' + data[id][1] + '<br>平均：' + data[id][2];
			}
		},
		radiusAxis: {
		},
...
function updateAll(selectRegion,selectTime, selectIndust){

	$.post("govPolicyMacroRegion/comInfo", {'region':selectRegion, 'time':selectTime, 'industType':selectIndust }, function(ret){
		legendData = ret['legendData']
		xAxisData = ret['xAxisData']
		comNumSData = ret['comNumSData']
		comCapitalSData = ret['comCapitalSData']
		taxReliefSData = ret['taxReliefSData']
		comIncomeSData = ret['comIncomeSData']
		taxReliefRegionSum = ret['taxReliefRegionSum']
		comIncomeRegionSum = ret['comIncomeRegionSum']
		increaseRegionSum = ret['increaseRegionSum']
		increaseSData = ret['increaseSData']
		increaseRegionSumMin = ret['increaseRegionSumMin']
		increaseRegionSumMax = ret['increaseRegionSumMax']

		updateBarChartY(xAxisData, legendData, comNumSData, 'chart1');
		updateRadarChart(xAxisData, legendData, comCapitalSData, 'chart3');
		updateBarChart(xAxisData, legendData, taxReliefSData, 'chart4');
		updateBarChart(xAxisData, legendData, comIncomeSData, 'chart5');
		updateBarChart1(xAxisData, legendData, increaseSData, 'chart6');

		updatePieChart(xAxisData, legendData, taxReliefRegionSum, taxReliefSData, 'chart7', '税收减免');
		updatePieChart(xAxisData, legendData, comIncomeRegionSum, comIncomeSData, 'chart8', '营收情况');

		updateRadarChart1(xAxisData, legendData, increaseRegionSumMin, increaseRegionSumMax, increaseRegionSum, 'chart9'); 
...
//}
//else{
//	updateChart1(selectRegion, selectTime, selectIndust);
//}

updateAll(selectRegion,selectTime, selectIndust);
